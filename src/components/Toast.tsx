...
function Toast(props: ToastProps) {
  ...
  return (
    <div className="flex w-md items-stretch gap-3 rounded-md bg-accent p-4">
      <div
        className={cn(
          "mt-0.5 grid size-8 place-items-center rounded-md shrink-0",
          styles[resolvedVariant].badge,
        )}
      >
        <IconComp
          className={cn("size-5", styles[resolvedVariant].iconColor)}
          aria-hidden="true"
        />
      </div>
      ...
      <div className="flex flex-1 flex-row justify-between gap-4 overflow-hidden">
        <div className="flex h-full min-w-0 flex-col items-start justify-center">
          <p className="w-full truncate text-sm leading-none font-medium">
            {title}
          </p>
          {description ? (
            <p className="mt-1 line-clamp-3 w-full text-sm text-muted-foreground">
              {description}
            </p>
          ) : null}
        </div>
